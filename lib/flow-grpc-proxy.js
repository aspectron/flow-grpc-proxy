...
class FlowGRPCProxy extends EventEmitter{
		...
		this.websockets = this.io.of(websocketPath).on('connection', async (socket)=>{
			...
			*/
			...
			if(websocketMode == 'NATS') {
				...
				const jc = FlowGRPCProxy.modules.NATS.JSONCodec();
				...
				socket.on('request', (msg) => {
					...
					this.nats.request(subject, jc.encode(data)).then((msg) => {
						...
						this.handleResponse(socket.id, session.user, subject, data, session);
	...
	initStaticFiles(){
		let {express} = FlowGRPCProxy.modules;
		console.log("initStaticFiles", this.config.staticFiles)
