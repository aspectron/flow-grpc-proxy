...
class FlowGRPCProxy extends EventEmitter{
	...
	initIO(io){
		...
		if(this.config.websocketPath)
			this.init_socketio_handler(this.config.websocketPath, this.config.websocketMode || 'GRPC');
	}
		...
		this.websockets = this.io.of(websocketPath).on('connection', async (socket)=>{
			...
			*/
			...
			socket.on('grpc.request', (msg) => {
				...
				let { req : { client:clientName, subject, data }, rid } = msg;
				this.debug && console.log('got grpc.request:', subject, '->', msg);
				...
				if(!data)
					data = { };
				if(!this.checkAuth(session.user, subject, data, FlowGRPCProxy.METHODS.REQUEST)) {
				...
				console.log(`typeof this.grpc.${clientName}.${subject}`, typeof client[subject])

				if(typeof client[subject] != "function"){
					socket.emit('grpc.response', { rid, error: `${subject} function not found` });
					return
				}
				client[subject](data, (error, response) => {

					this.debug && console.log("+++++++++++++++++++++++++++++++++++++++++");
					this.debug && console.log('got response:', subject, '->', error, response);
					if(!error)
						this.handleResponse(socket.id, session.user, subject, response, session);
	...
	initStaticFiles(){
		let {express} = FlowGRPCProxy.modules;
		if(!this.config.staticFiles || !express)
			return
